...
With --decrypt_mode, it decrypts the stdin input and output the plaintext using the private key fetched from Azure Protected Audience KMS.
The above json format is used for the input.
...
namespace privacy_sandbox::azure_encrypt_payload
{
  namespace
  {
        ...
        const std::string plaintext_payload)
    {
      ...
      std::string ciphertext_base64;
        CHECK(privacy_sandbox::azure_encrypt_payload::Base64Encode(ciphertext_bytes, ciphertext_base64).Successful())
        ...
        const EncryptPayloadResult& encrypt_payload_result)
    {
        ...
        CHECK(privacy_sandbox::azure_encrypt_payload::Base64Decode(encrypt_payload_result.ciphertext_base64, ciphertext_bytes).Successful())
            << "Failed to encode ciphertext as base64";

        server_common::EncapsulatedRequest encapsulatedRequest = {ciphertext_bytes,
                                                                label_for_test};

        absl::StatusOr<quiche::ObliviousHttpRequest> decrypted_result =  server_common::DecryptEncapsulatedRequest(private_key.value(),
                                                        encapsulatedRequest);
...
int main(int argc, char **argv)
{
    ...
    auto key_fetcher_manager = privacy_sandbox::azure_encrypt_payload::CreateKeyFetcherManager(
        privacy_sandbox::azure_encrypt_payload::CreatePublicKeyFetcher());

    bool decrypt_mode = {
          absl::GetFlag(FLAGS_decrypt_mode)};
    
    if (! decrypt_mode) {
        std::string plaintext_payload;
        std::cin >> plaintext_payload;
        const auto encrypt_result =
            privacy_sandbox::azure_encrypt_payload::EncryptPayload(std::move(key_fetcher_manager), plaintext_payload);
        ...
        nlohmann::json input;
        std::cin >> input;
        auto encryption_result = input.template get<privacy_sandbox::azure_encrypt_payload::EncryptPayloadResult>();
        const auto output_path = absl::GetFlag(FLAGS_output_path);
        // We can improve this by checking the directly of the file exists.
        // Currently it silently fails to write if the dir is not there.
        std::ofstream fout(output_path);
        // fout << "decryption can be done with key ID " << static_cast<int>(encryption_result.key_id) << ". input: " << nlohmann::json(input).dump(2) << std::endl;
        fout << DecryptCiphertext(std::move(key_fetcher_manager), encryption_result);
    }
