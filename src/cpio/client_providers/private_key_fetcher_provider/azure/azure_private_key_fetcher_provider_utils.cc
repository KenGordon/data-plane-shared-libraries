...
AzurePrivateKeyFetchingClientUtils::GenerateWrappingKey() {
  RsaWrapper rsa;
  BnWrapper e;
  std::cout << "===============>AzurePrivateKeyFetchingClientUtils::"
               "GenerateWrappingKey"
            << std::endl;
  ERR_clear_error();
  BN_set_word(e.get(), RSA_F4);
  std::cout << "===============>AzurePrivateKeyFetchingClientUtils::"
               "GenerateWrappingKey after BN_set_word"
            << std::endl;
  ...
  if (EVP_PKEY_set1_RSA(private_key->get(), rsa.get()) != 1) {
    std::cout << "===============>AzurePrivateKeyFetchingClientUtils::"
                 "GenerateWrappingKey Getting private EVP_PKEY failed"
              << std::endl;
  ...
  if (!RSA_set0_key(rsa_pub_dup.get(), rsa_pub->n, rsa_pub->e, NULL)) {
    std::cout
        << "===============>AzurePrivateKeyFetchingClientUtils::"
           "GenerateWrappingKey Set RSA public key duplicate values failed"
        << std::endl;
    char errBuffer[MAX_OPENSSL_ERROR_STRING_LEN];
    char* error_string = ERR_error_string(ERR_get_error(), errBuffer);
    ERR_clear_error();
  ...
  if (EVP_PKEY_set1_RSA(public_key->get(), rsa_pub_dup.get()) != 1) {
    std::cout << "===============>AzurePrivateKeyFetchingClientUtils::"
                 "GenerateWrappingKey Set RSA public key failed"
              << std::endl;
    char errBuffer[MAX_OPENSSL_ERROR_STRING_LEN];
    char* error_string = ERR_error_string(ERR_get_error(), errBuffer);
    ERR_clear_error();
  ...
  std::cout << "===============>AzurePrivateKeyFetchingClientUtils::"
               "GenerateWrappingKey return key"
            << std::endl;
