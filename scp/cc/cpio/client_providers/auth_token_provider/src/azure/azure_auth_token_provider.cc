...
namespace {
constexpr char kAadAuthTokenProvider[] = "AadAuthTokenProvider";
...
AadAuthTokenProvider::AadAuthTokenProvider(
    const std::shared_ptr<HttpClientInterface>& http_client)
    : http_client_(http_client) {}

ExecutionResult AadAuthTokenProvider::Init() noexcept {
  if (!http_client_) {
    auto execution_result = FailureExecutionResult(
        SC_AZURE_INSTANCE_AUTHORIZER_PROVIDER_INITIALIZATION_FAILED);
    SCP_ERROR(kAadAuthTokenProvider, kZeroUuid, execution_result,
...
ExecutionResult AadAuthTokenProvider::Run() noexcept {
  return SuccessExecutionResult();
}

ExecutionResult AadAuthTokenProvider::Stop() noexcept {
  return SuccessExecutionResult();
}

ExecutionResult AadAuthTokenProvider::GetSessionToken(
    AsyncContext<GetSessionTokenRequest, GetSessionTokenResponse>&
        get_token_context) noexcept {
  // Make a request to the metadata server:
  // The Application is running on a AAD VM which runs as a service account.
  // Services which run on AAD also spin up a local metadata server which can be
  ...
  http_context.callback =
      absl::bind_front(&AadAuthTokenProvider::OnGetSessionTokenCallback, this,
                       get_token_context);

  auto execution_result = http_client_->PerformRequest(http_context);
  if (!execution_result.Successful()) {
    SCP_ERROR_CONTEXT(kAadAuthTokenProvider, get_token_context,
                      execution_result,
...
void AadAuthTokenProvider::OnGetSessionTokenCallback(
    AsyncContext<GetSessionTokenRequest, GetSessionTokenResponse>&
        get_token_context,
    AsyncContext<HttpRequest, HttpResponse>& http_client_context) noexcept {
  if (!http_client_context.result.Successful()) {
    SCP_ERROR_CONTEXT(
        kAadAuthTokenProvider, get_token_context, http_client_context.result,
  ...
  try {
    ...
    auto result = RetryExecutionResult(
        SC_AZURE_INSTANCE_AUTHORIZER_PROVIDER_BAD_SESSION_TOKEN);
    SCP_ERROR_CONTEXT(
        kAadAuthTokenProvider, get_token_context, result,
  ...
  // The life time of AAD access token is about 1 hour.
...
ExecutionResult AadAuthTokenProvider::GetSessionTokenForTargetAudience(
    AsyncContext<GetSessionTokenForTargetAudienceRequest,
                 GetSessionTokenResponse>& get_token_context) noexcept {
  // Make a request to the metadata server:
  // The PBS is running on a AAD VM which runs as a service account. Services
  // which run on AAD also spin up a local metadata server which can be
  // queried for details about the system. curl -H "Metadata-Flavor: Google"
  ...
  http_context.callback = absl::bind_front(
      &AadAuthTokenProvider::OnGetSessionTokenForTargetAudienceCallback, this,
      get_token_context);

  return http_client_->PerformRequest(http_context);
}

void AadAuthTokenProvider::OnGetSessionTokenForTargetAudienceCallback(
    AsyncContext<GetSessionTokenForTargetAudienceRequest,
  ...
  if (token_parts.size() != kExpectedTokenPartsSize) {
    auto result = RetryExecutionResult(
        SC_AZURE_INSTANCE_AUTHORIZER_PROVIDER_BAD_SESSION_TOKEN);
    SCP_ERROR_CONTEXT(kAadAuthTokenProvider, get_token_context, result,
  ...
  if (auto decode_result = Base64Decode(*padded_jwt_or, decoded_json_str);
    ...
    SCP_ERROR_CONTEXT(kAadAuthTokenProvider, get_token_context, decode_result,
  ...
  // token is fetched from the AAD platform close to the VM where this code will
...
std::shared_ptr<AuthTokenProviderInterface> AuthTokenProviderFactory::Create(
  ...
  return std::make_shared<AadAuthTokenProvider>(http1_client);
}
}  // namespace google::scp::cpio::client_providers
